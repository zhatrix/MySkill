...
| `codev_opinion_add <repo> <doc> <round> <agent> <model> <role> <issue> <stance> <severity> <fix> <note>` / `codev_opinions [issue]` | 意见与判断记录（`CODEV_OPINIONS`，默认 `~/.local/state/codev/opinions.tsv`，12 列）：一个主体对一个问题的一条立场一行，role=评审/判断/自审/复核，stance=提出/采纳/驳回/存疑/未返回，severity=P1/P2/P3/-。发现台账只装最终裁决，多位判断者各自的级别与修法存不下——这里保留原始立场。`codev_opinions` 按问题回放并判出 一致采纳同一修法／一致驳回／修法不同(disputed)／未达成一致；缺席**必须显式记 `未返回`**，留空在回放里等于“没人反对”。三个枚举列当场校验（同 `finding_add` 的教训：写错会被静默计 0）。 |
